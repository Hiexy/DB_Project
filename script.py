...
def insert(t_option):
    ...
    if t_option == 2:
        ...
        try:
            query="INSERT INTO store (location, contract_start,contract_end ,name,space,rented,visibility, sales,rental_fee,Floor_id,Business_id) VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s) RETURNING id"
            val = (loc, con_start, con_end, name, space, rented, vis, sales, fid, bid)
            id = cur.execute(query, val)
            upfee(id)
            conn.commit()
        except:
            print("Insert query not executed")
